...
// prettyDump($_POST);
    if (isset($_POST['submit']) && $_POST['submit'] != '') {
        ...
        $query1 = $con->prepare("UPDATE product SET name = ?, description = ?, price = ?, color = ?, weight = ?, active = ? WHERE product_id = ? LIMIT 1;");
        if ($query1 === false) {
            echo mysqli_error($con);
        }
                    
        $query1->bind_param('ssssssi',$name,$description,$price,$color,$weight,$active,$id);
